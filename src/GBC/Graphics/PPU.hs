...
startOutput :: Memory -> VRAM -> GraphicsSync -> IO SDL.Window
...
renderLine :: Word8 -> VRAM -> Ptr Word8 -> Ptr Int8 -> Ptr Word8 -> ReaderT Memory IO ()
renderLine line vram assemblySpace priorityBuffer outputBase = do
  scx  <- readByte SCX
  scy  <- readByte SCY
  wx   <- readByte WX
  wy   <- readByte WY
  ...
  when bgEnabled $ do
    ...
    let bgTiles = if lcdc .&. flagBackgroundTileMap == 0 then 0x1800 else 0x1C00
  ...
  when windowEnabled $ do
    ...
    let windowTiles    = if lcdc .&. flagWindowTileMap == 0 then 0x1800 else 0x1C00
   ...
   where
    go !inPos !outPos = if outPos >= stopOffset
      then pure ()
        ...
        (byte0, byte1) <- readTileData vram (tileDataBase + fontLineOffset)
  ...
  doSprite h offset = do
    ...
    if not spriteVisible
      then pure False
        ...
        (byte0, byte1) <- readTileData vram fontLineOffset
