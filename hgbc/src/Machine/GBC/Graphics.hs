...
renderLine GraphicsState {..} line assemblySpace priorityBuffer outputBase = do
  ...
  when bgEnabled $ do
    ...
    tileMachine mode bgTileLine fontOffset yOffset windowStart xTile (negate xOffset)
  ...
  when windowEnabled $ do
    ...
    tileMachine mode windowTileLine fontOffset yOffsetWindow 160 0 windowStart

  -- Render sprite data to the assembly area.
  let spriteHeight = if isFlagSet flagOBJSize lcdc then 16 else 8
  when spritesEnabled $ doSprites mode spriteHeight
 ...
 where
  ...
  tileMachine !mode !tileBase !fontOffset !yOffset !stopOffset = go
   where
