...
data TestTree summary result
  = TestCase Name Required result
...
generateReport title testTime reportTime results =
  BB.toLazyByteString
  ...
  where
    ...
    reportResultTree parent level (TestTree name sourceLink (passed, failed) subtrees) =
      let nextParent = Just (maybe name (</> name) parent)
          summaryClass
            | passed == 0 = "failed"
            | failed == 0 = "passed"
            | otherwise = "mixed"
       in ( ("<h" <> BB.intDec level <> " id='" <> makeAnchor parent name <> "'>")
              <> BB.stringUtf8 name
              <> ("</h" <> BB.intDec level <> ">")
          )
            <> ( if isNothing parent
